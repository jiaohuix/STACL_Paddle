...
def get_arguments(return_parser=False):
    ...
    parser.add_argument("--waitk", default=None, type=int,help="Waitk policy.")
    parser.add_argument("--stream", action="store_true",help="Weather to use stream input when generation.")
    parser.add_argument("--stream-bpe", action="store_true",help="Weather to use bpe when stream generation.")
...
def get_config(args):
    ...
    # simultaneous translation
    if args.waitk:
        conf.waitk = args.waitk
    if args.stream:
        conf.generate.stream = args.stream
    if args.stream_bpe:
        conf.generate.use_stream_bpe=args.stream_bpe
    # distributed training parameters
    if args.amp:
