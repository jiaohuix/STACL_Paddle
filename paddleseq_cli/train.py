...
from reader import prep_dataset, prep_loader
from paddleseq_cli.utils import same_seeds, save_model, \
    ReduceOnPlateauWithAnnael, NMTMetric, get_logger, get_grad_norm,InverseSquareRoot
...
def train_one_epoch(conf,
    ...
    for batch_id, input_data in enumerate(dataloader):
        ...
        if (conf.train.save_step>0) and (step_id % conf.train.save_step == 0) and (dist.get_rank() == 0):
            save_model(model, optimizer, save_dir=os.path.join(conf.SAVE, conf.model.save_model, f"step_{step_id}"))

        if isinstance(scheduler,
                      (LinearDecayWithWarmup, CosineAnnealingDecay, NoamDecay,InverseSquareRoot)):  # these scheds  updated each step
...
def get_optimizer_scheduler(conf,model,dataloader,global_step_id):
    ...
    if learn_args.sched == "plateau":
        ...
        scheduler = NoamDecay(d_model=sched_args.d_model,
                              warmup_steps=learn_args.scheduler.warmup.warm_steps,
    ...
    assert scheduler is not None, "Sched should in [plateau|cosine|warmup|noamdecay|inverse_sqrt]."
...
def early_stop(conf,optimizer,val_loss ,lowest_val_loss,num_runs,gnorm,logger):
    stop_flag=False
...
def main_worker(*args):
    ...
    train_loader = None
    if not conf.eval:
        train_loader = prep_loader(conf, dataset_train, mode="train")
        logger.info(f"----- Total of train set:{len(train_loader.dataset)} ,train batch: {len(train_loader)} [single gpu]")
    dev_loader = prep_loader(conf, dataset_val, mode="dev")
    toc=time.time()
    ...
    best_bleu = 0
    if local_rank == 0:
        strtime = time.strftime("Time%m%d_%Hh%Mm", time.localtime(time.time()))  # 注意冒号会保存不了
    ...
    for epoch in range(last_epoch + 1, conf.train.max_epoch + 1):
        ...
        if (best_bleu < dev_bleu) and (local_rank == 0):
            ...
            save_model(model, optimizer, save_dir=save_dir)
        ...
        # early stop
        stop_flag,lowest_val_loss,num_runs=early_stop(conf,optimizer,val_loss ,lowest_val_loss,num_runs,gnorm,logger)
        if stop_flag:break
