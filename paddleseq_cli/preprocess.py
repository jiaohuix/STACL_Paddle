...
from reader.binarizer import Binarizer
from reader import indexed_dataset
from reader.file_chunker_utils import find_offsets
...
def get_vocab_path(args,return_src=True):
    ...
    # 得到字典
    if args.joined_dictionary:  # 联合字典（src和tgt）
        assert (
...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers): # output_prefix就没有1
        ...
        # 以上结束生成idx吗，idx i再哪里生成
        if num_workers > 1:
            pool.join()
            for worker_id in range(1, num_workers):
                prefix = "{}{}".format(output_prefix, worker_id) # 前缀添加workid
